...
| `user_status` | `{ userId: string, status: 'online' \| 'offline' }` | Presence update for a friend. Delivered when a friend connects or disconnects, when the instance holding their last session shuts down gracefully, and when that instance stops without shutting down (SIGKILL, OOM, a reclaimed container). In that last case the instance itself sends nothing: every instance still running checks on each presence heartbeat whether a friend of one of its own connected users has lost their last lease, and announces `offline` to its own sessions only — within `PRESENCE_TTL_MS` plus one heartbeat of the termination (about 40 s at the defaults, plus the brief delay Redis takes to reclaim the expired field). Connects and disconnects are addressed to every friend's `user_<id>` room, so they reach their sessions on any instance, and while Redis is reachable each is announced for the whole cluster by the instance that took the first lease or released the last one. The heartbeat check has two side effects, so clients must treat `user_status` as idempotent: an ordinary disconnect is normally followed, one heartbeat later, by the same `offline` again from every instance where one of the user's friends is connected; and for up to `2 × PRESENCE_TTL_MS` after an unfriend or a block, the other party may receive one more `offline` about you, because the check caches friend lists for that long. **Degraded case:** if the Redis command connection is down, each instance falls back to its own local view — so a client may receive a duplicate `online`, or an `offline` while the user is still connected to another instance. The heartbeat check announces nothing while Redis cannot answer it, and after an outage waits one more heartbeat before comparing again, so an instance that terminated during the outage is not announced and its users stay `online` until their friends' next `GET /api/v1/friends`. `GET /api/v1/friends` does not correct this during the outage: it derives `status` from the same presence lookup, which also falls back to the answering instance's own connections and so reports a user connected elsewhere as offline. Both paths converge once Redis is reachable again. **Recovery:** if an instance's presence lease lapses while its own sockets stay up — a command-connection outage longer than `PRESENCE_TTL_MS` — the other instances' heartbeat check announces its users `offline`, and the first heartbeat after the connection recovers re-takes the lease and announces `online` again, so those clients are corrected without waiting for their next `GET /api/v1/friends`. |
