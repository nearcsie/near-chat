...
| `NOT_FOUND` | 404 | 資源不存在 |
| `FORBIDDEN` | 403 | 無操作權限 |
| `CONFLICT` | 409 | 資源衝突（如重複的好友邀請、email 已被使用，或 `If-Match` revision 過期）。能否重試取決於衝突本身：revision 過期在客戶端重新取得訊息後可以成功，資源已存在的衝突則不會 |
...
- **不可用的 cursor**: 當 cursor 落在 change log 目前涵蓋的範圍之外——低於最舊的序號（重新 seed 後的狀態），或高於最新的序號（還原較舊備份後的狀態）——代表這個 cursor 是對一份伺服器已不再持有的 log 發出的，之後的任何 delta 都不可能把它往前帶。該頁最尾端的序號同樣受此上界檢查，因此在請求進行中落地的還原不會把 log 已不再涵蓋的序號交出去。此時回應為 `{ "changes": [], "nextCursor": 0, "hasMore": false, "resyncRequired": true }`，且該頁變更不會交出，以免客戶端越過這個斷點。客戶端會從 `0` 重新開始；它手上既有的歷史可能含有已被重置移除的訊息，因此也應丟棄該份歷史並改以聊天室端點重新取得——目前的 Web 客戶端只重設 cursor，「以 `listMessages` 取代而非合併房間歷史」由 #679 追蹤，在那之前一次重置可能讓畫面上留下伺服器已不再持有的訊息。其餘情況一律不會出現 `resyncRequired`；單純已追上的 cursor 仍然是原值回傳、變更為空。
...
| `user_status` | `{ userId: string, status: 'online' \| 'offline' }` | 好友的上線 / 下線狀態更新。於好友連線或斷線時推送，於持有其最後一條 session 的 instance 正常關機時推送，也於該 instance 未經關機程序即終止（SIGKILL、OOM、container 被回收）時推送。最後這種情況不會由該 instance 本身送出：仍在運作的每個 instance 會在每次 presence heartbeat 檢查「自己本機連線使用者的好友」是否已失去最後一個 lease，並只對自己的 session 宣告 `offline`——在終止後 `PRESENCE_TTL_MS` 加一個 heartbeat 週期內送達（預設約 40 秒，另加 Redis 回收過期欄位的短暫延遲）。連線與斷線事件會送往每位好友的 `user_<id>` room，因此能送達他們在任一 instance 上的 session；在 Redis 可連線的情況下，這兩種轉換由取得第一個 lease 或釋放最後一個 lease 的那個 instance 對整個 cluster 宣告。heartbeat 檢查帶來兩項副作用，因此客戶端必須將 `user_status` 視為冪等：一般斷線之後，通常會在一個 heartbeat 後，由每個有該使用者好友連線的 instance 再送一次相同的 `offline`；解除好友或封鎖後最多 `2 × PRESENCE_TTL_MS` 內，對方仍可能再收到一次關於你的 `offline`，因為該檢查會快取好友清單這麼久。**降級情境：** 若 Redis command connection 中斷，各 instance 會退回依自己的本機狀態判斷，因此客戶端可能收到重複的 `online`，或在使用者仍連線於其他 instance 時收到 `offline`。heartbeat 檢查在 Redis 無法回應期間不做任何宣告，恢復後也會多等一個 heartbeat 才重新比對，因此中斷期間終止的 instance 不會被宣告，其使用者會停留在「在線」，直到好友下一次 `GET /api/v1/friends`。中斷期間 `GET /api/v1/friends` 無法用來校正：其 `status` 來自同一組 presence 查詢，同樣會退回成回應端 instance 自己的連線，因而把連在其他 instance 的使用者回報為 offline。兩條路徑都要等 Redis 恢復連線後才會收斂。**恢復：** 若某個 instance 的 socket 仍然健康，presence lease 卻因 command connection 中斷超過 `PRESENCE_TTL_MS` 而過期，其他 instance 的 heartbeat 檢查會把它的使用者宣告為 `offline`；連線恢復後的第一次 heartbeat 會重新取得 lease 並補宣告一次 `online`，因此這些客戶端不必等到下一次 `GET /api/v1/friends` 才會被更正。 |
