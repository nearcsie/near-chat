        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-border-secondary">
          <RequestColumn
            ...
            renderActions={(requestId) => (
              <>
                <Button type="button" variant="primary" className="text-xs py-1 px-2" onClick={() => void acceptFriendRequest(requestId).catch(console.error)}>
                  {t("friends.accept")}
                </Button>
                <Button type="button" variant="secondary" className="text-xs py-1 px-2" onClick={() => void rejectFriendRequest(requestId).catch(console.error)}>
                  {t("friends.reject")}
                </Button>
              </>
            )}
        ...
        <div className="divide-y divide-border-secondary">
          {blockedUsers.map((user) => (
            <div key={user.id} className="p-4 flex items-center justify-between gap-4">
              ...
              <Button type="button" variant="secondary" className="text-xs py-1 px-2" onClick={() => void unblockUser(user.id).catch(console.error)}>
                {t("friends.unblock")}
              </Button>
            </div>
          ))}
