...
class UserRepository {
    ...
    async search(query) {
        const res = await this.db.query(`SELECT * FROM users WHERE name ILIKE $1 OR user_id::text = $2 LIMIT 20`, [`%${query}%`, query]);
        return res.rows.map(mapRowToUser);
    }
    async create(data) {
        const res = await this.db.query(`INSERT INTO users (name, email, password_hash)
       VALUES ($1, $2, $3)
       RETURNING *`, [data.name, data.email, data.passwordHash]);
        return mapRowToUser(res.rows[0]);
    }
